...
class BlockDevice:
	...
	def json(self):
		"""
		json () tem precedência sobre __dump__, então esta é uma maneira
        para fornecer menos informações / parciais para a legibilidade do usuário.
		"""
		return {
			'path': self.path,
	...
	def device(self):
		"""
		Retorna o ponto final do dispositivo real do BlockDevice.
        Se for um dispositivo de loopback, ele retorna o arquivo de volta,
        Se for um drive ATA ele retorna o dispositivo / dev / X
        E se for um dispositivo criptográfico, ele retorna o dispositivo pai
		"""
		if "type" not in self.info:
			raise DiskError(f'Não foi possível localizar as informações do painel traseiro para "{self.path}"')
		...
		if self.info['type'] == 'loop':
			...
			if 'pkname' not in self.info:
				raise DiskError(f'Um dispositivo de cripta ({self.path}) sem um nome de dispositivo de kernel pai.')
	...
	def partitions(self):
		...
		if b'not a block device' in o:
			raise DiskError(f'Não é possível ler partições de algo que não seja um dispositivo de bloco: {self.path}')

		if not o[:1] == b'{':
			raise DiskError('Erro ao obter saída JSON de:', f'/usr/bin/lsblk -J {self.path}')
	...
	@property
	def uuid(self):
		log('BlockDevice (). Uuid não foi testado!', level=logging.WARNING, fg='yellow')
		"""
		Retorna o UUID do disco conforme retornado por lsblk.
        Isso é mais confiável do que confiar em / dev / disk / by-partuuid como
        ele não parece ser capaz de detectar partições do MD RAID.
...
class Partition:
	def __init__(self, path: str, block_device: BlockDevice, part_id=None, size=-1, filesystem=None, mountpoint=None, encrypted=False, autodetect_filesystem=True):
		...
		if self.mountpoint != mount_information.get('target', None) and mountpoint:
			raise DiskError(f"{self} recebeu um ponto de montagem, mas o ponto de montagem real é diferente: {mount_information.get('target', None)}")
	...
	def uuid(self) -> Optional[str]:
		"""
		Retorna o PARTUUID conforme retornado por lsblk.
        Isso é mais confiável do que confiar em / dev / disk / by-partuuid como
        ele não parece ser capaz de detectar partições do MD RAID.
	...
	def detect_inner_filesystem(self, password):
		log(f'Tentando detectar o formato do sistema de arquivos interno em {self} (Isso pode demorar um pouco)', level=logging.INFO)
	...
	def has_content(self):
		...
		if (handle := SysCommand(f'/usr/bin/mount {self.path} {temporary_mountpoint}')).exit_code != 0:
			raise DiskError(f'Não foi possível montar e verificar o conteúdo em: {self.path} por causa do: {b"".join(handle)}')
	...
	def safe_to_format(self):
		if self.allow_formatting is False:
			log(f"Partiçao {self} não está marcado para formatação.", level=logging.DEBUG)
			return False
		elif self.target_mountpoint == '/boot':
			try:
				if self.has_content():
					log(f"Partiçao {self} é uma partição de inicialização e contém conteúdo.", level=logging.DEBUG)
					return False
			except SysCallError as err:
				log(err.message, logging.DEBUG)
				log(f"Partiçao {self} foi identificado como /boot, mas não foi possível montar para verificar o conteúdo, continuando!", level=logging.DEBUG)
				pass
	...
	def encrypt(self, *args, **kwargs):
		"""
		Uma função de wrapper para instâncias luks2 () e o método .encrypt () dessa instância.
		"""
		from .luks import luks2
		...
		if not self.safe_to_format():
			log(f"Partiçao {self} foi marcado como protegido, mas encrypt () foi chamado nele!", level=logging.ERROR, fg="red")
			return False
