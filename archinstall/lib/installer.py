...
class Installer:
	...
	def enable_service(self, *services):
		for service in services:
			...
			if (output := self.arch_chroot(f'systemctl enable {service}')).exit_code != 0:
				raise ServiceException(f"Incapaz de iniciar o serviço {service}: {output}")
	...
	def set_x11_keyboard_language(self, language: str) -> bool:
		...
		if len(language.strip()):
			...
			self.log(f'O idioma do teclado X11 não foi alterado do padrão (nenhum idioma especificado).', fg="yellow", level=logging.INFO)
