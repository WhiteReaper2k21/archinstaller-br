...
archinstall.log("Minimal suporta apenas:")
archinstall.log("* Sendo instalado em um único disco")

if archinstall.arguments.get('help', None):
	archinstall.log("- Criptografia de disco opcional via -! Encryption-password = <password>")
	archinstall.log("- Tipo de sistema de arquivos opcional via --filesystem = <fs type>")
	archinstall.log("- Rede systemd opcional via --network")
...
def install_on(mountpoint):
	...
	archinstall.log("Existem duas novas contas em sua instalação após a reinicialização:")
