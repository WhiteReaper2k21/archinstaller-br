...
if archinstall.arguments.get('help'):
	print("Veja `man archinstall` para obter ajuda.")
	exit(0)
if os.getuid() != 0:
	print("Archinstall requer privilégios de root para ser executado. Veja --help para mais.")
	exit(1)

# Log various information about hardware before starting the installation. This might assist in troubleshooting
archinstall.log(f"Modelo de hardware detectado: {archinstall.sys_vendor()} {archinstall.product_name()}; UEFI mode: {archinstall.has_uefi()}", level=logging.DEBUG)
archinstall.log(f"Modelo de processador detectado: {archinstall.cpu_model()}", level=logging.DEBUG)
archinstall.log(f"Estatísticas de memória: {archinstall.mem_available()} available out of {archinstall.mem_total()} total installed", level=logging.DEBUG)
archinstall.log(f"Virtualização detectada: {archinstall.virtualization()}; is VM: {archinstall.is_vm()}", level=logging.DEBUG)
archinstall.log(f"Dispositivos gráficos detectados: {archinstall.graphics_devices().keys()}", level=logging.DEBUG)
...
def ask_user_questions():
	"""
		Primeiro, pediremos ao usuário uma série de informações do usuário.
        Não até que estejamos satisfeitos com o que queremos instalar
        continuaremos com as etapas reais de instalação.
	...
	if not archinstall.arguments.get('sys-language', None) and archinstall.arguments.get('advanced', False):
		archinstall.arguments['sys-language'] = input("Insira uma localidade válida (idioma) para o seu sistema operacional, (Padrão: en_US").strip()
		archinstall.arguments['sys-encoding'] = input("Insira uma codificação padrão do sistema válida para o seu sistema operacional (Padrão: utf-8): ").strip()
		archinstall.log("Lembre-se de que, se você quiser vários locais, a pós-configuração é necessária.", fg="yellow")
	...
	if archinstall.arguments['harddrive'] and archinstall.arguments['harddrive'].has_partitions():
		archinstall.log(f"{archinstall.arguments['harddrive']} contém as seguintes partições:", fg='yellow')
